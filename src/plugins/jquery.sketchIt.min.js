function sketchIt(e, n) {
  ...
  this.$me
    .on('vmousedown', function () {
      if (t.mode != 'arrow') {
        ...
        t.maxClicks++
        t.draw(
          { mode: 'start', x: event.pageX - o.left, y: event.pageY - o.top, clickId: t.clicks },
          true,
        )
      }
    })
  ...
  if (n.tools) {
    ...
    $('body').on('click', '[data-control="#' + this.$me.attr('id') + '"]', function (e) {
...
sketchIt.prototype = {
  ...
  changeSetting: function (e, t) {
    if (this.settings.tools) {
      $('[data-' + e + ']').removeClass('current')
      $('[data-' + e + '="' + t + '"]').addClass('current')
      $('[data-category="' + e + '"] .catview').remove()
      $('[data-category="' + e + '"]').prepend(
        '<span class="catview">' + $('[data-' + e + '="' + t + '"]').html() + '</span>',
      )
    }
    log = { mode: 'set', key: e, val: t }
    this.set(log, true)
  },
  set: function (e, t) {
    switch (e.key) {
      case 'color':
        this.ctx.strokeStyle = e.val
        break
      case 'width':
        this.ctx.lineWidth = e.val
        break
      case 'cap':
        this.ctx.lineCap = e.val
        this.ctx.lineJoin = e.val
        break
      case 'fill':
        this.ctx.fillStyle = e.val
        break
      case 'tool':
        if (e.val == 'arrow') {
        ...
        cursor = e.val == 'arrow' ? 'auto' : 'crosshair'
  ...
  clearCanvas: function (e) {
    ...
    this.ctx.restore()
    if (e) {
      this.history.push({ mode: 'clear' })
    }
  },
}
;(function (e, t, n, r) {
  e.fn.sketchIt = function (t) {
    var n = e.extend({ color: '#f00', width: 5, cap: 'round', mode: 'pen', tools: true }, t)
